"use strict";
const VERSION="1.1", STORAGE_KEY="clickset_017_groups";
...
function cancelScheduledAudio(){
 runId++;clearTimeout(schedulerTimer);schedulerTimer=null;trackLoadToken++;
...
function loadSampleBuffers(){
 ...
 Object.entries(SAMPLE_FILES).forEach(([id,files])=>Object.entries(files).forEach(([kind,file])=>{
  ...
  jobs.push(fetch(`./${file}?v=11`,{cache:"force-cache"}).then(r=>{if(!r.ok)throw new Error(`${file}: ${r.status}`);return r.arrayBuffer()}).then(data=>audioCtx.decodeAudioData(data)).then(buffer=>{sampleBuffers[key]=buffer}).catch(e=>console.warn("Sample",file,e)))
 }));
...
async function start(immediateFirstBeat=true){
 ...
 await ensureAudio();
 const song=flat[current].song;
 playing=true;runId++;const myRun=runId;beatInBar=0;trackBuffer=null;
 ...
 const firstBeatTime=audioCtx.currentTime+(immediateFirstBeat?.004:.018);
 const accented=Boolean(song.accentFirst);
 scheduleBeatSound(firstBeatTime,accented);
 pulseAt(firstBeatTime,myRun);
 ...
 scheduler(myRun);
 // La càrrega de la pista no retarda mai el primer clic del metrònom.
 if(song.trackId){
  try{
...
function stop(){playing=false;cancelScheduledAudio();$("play").textContent="▶ PLAY";$("play").classList.remove("stop");if($("concertPlay")){ $("concertPlay").textContent="▶ PLAY";$("concertPlay").classList.remove("stop");$("concertPulse").classList.remove("on") }$("pulse").classList.remove("on")}
