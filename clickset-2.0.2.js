"use strict";
const VERSION="2.0.2", STORAGE_KEY="clickset_017_groups";
...
let clickRoute=localStorage.getItem(CLICK_ROUTE_KEY)||(IS_IOS?"LR":"L");
let trackRoute=localStorage.getItem(TRACK_ROUTE_KEY)||(IS_IOS?"LR":"R");
// Migració única: les versions anteriors deixaven el click només al canal L.
// A l'altaveu intern de l'iPhone això pot quedar mut o sonar de manera inconsistent.
if(IS_IOS&&!localStorage.getItem("clickset_ios_audio_migrated_202")){
 clickRoute="LR";trackRoute="LR";
 localStorage.setItem(CLICK_ROUTE_KEY,"LR");
 localStorage.setItem(TRACK_ROUTE_KEY,"LR");
 localStorage.setItem("clickset_ios_audio_migrated_202","1");
}
...
async function unlockAudioNow(showFeedback=false){
 ...
 audioUnlockPromise=(async()=>{
  try{
   ...
   // Safari necessita que resume() passi dins el gest físic de l'usuari.
   if(ctx.state!=="running")await ctx.resume();
   if(ctx.state!=="running"){
    await new Promise(r=>setTimeout(r,30));
    await ctx.resume();
   }
   // Desbloqueig complementari de la ruta multimèdia d'iOS.
   const bridge=$("iosAudioBridge");
   if(bridge){
    try{
     bridge.volume=.02;bridge.currentTime=0;
     const p=bridge.play();if(p&&typeof p.then==="function")await p;
     bridge.pause();bridge.currentTime=0;
    }catch(e){console.warn("Pont d'àudio iOS",e)}
   }
   // Impuls Web Audio inaudible per mantenir el context inicialitzat.
   const buffer=ctx.createBuffer(1,Math.max(1,Math.floor(ctx.sampleRate*.03)),ctx.sampleRate);
   const data=buffer.getChannelData(0);data[0]=.0001;
   const source=ctx.createBufferSource(),gain=ctx.createGain();gain.gain.value=.0001;
   source.buffer=buffer;source.connect(gain).connect(ctx.destination);source.start(0);
   configureStereoRouting();
   audioUnlocked=ctx.state==="running";
   updateAudioGate();
   await Promise.allSettled([loadClassicBuffer(),loadSampleBuffers()]);
   if(showFeedback&&audioUnlocked){
    // Prova audible immediata: confirma que la sortida física real funciona.
    const t=ctx.currentTime+.015;
    scheduleSound("classic",t,1,true);
    showAudioToast(IS_IOS?"🔊 So iPhone activat":"🔊 So activat");
   }
   return audioUnlocked;
  }catch(e){
   console.warn("No s'ha pogut desbloquejar l'àudio",e);
   audioUnlocked=false;updateAudioGate();
   if(showFeedback)showAudioToast("Revisa volum, mode silenci i torna a tocar ACTIVAR SO");
   return false;
...
function loadSampleBuffers(){
 ...
 Object.entries(SAMPLE_FILES).forEach(([id,files])=>Object.entries(files).forEach(([kind,file])=>{
  ...
  jobs.push(fetch(`./${file}?v=202`,{cache:"force-cache"}).then(r=>{if(!r.ok)throw new Error(`${file}: ${r.status}`);return r.arrayBuffer()}).then(data=>audioCtx.decodeAudioData(data.slice(0))).then(buffer=>{sampleBuffers[key]=buffer}).catch(e=>console.warn("Sample",file,e)))
 }));
...
function loadClassicBuffer(){
 ...
 classicLoading=fetch("./click.wav?v=202",{cache:"force-cache"})
...
// No intentam desbloquejar en qualsevol toc: a Safari això pot consumir el gest abans del PLAY.
// PLAY i ACTIVAR SO criden ensureAudio/unlockAudioNow directament dins el toc de l'usuari.
$("activateAudioBtn").addEventListener("click",async e=>{e.preventDefault();await unlockAudioNow(true)});
window.addEventListener("pageshow",async()=>{if(audioCtx&&audioCtx.state!=="running"){try{await audioCtx.resume()}catch(e){}audioUnlocked=audioCtx.state==="running";updateAudioGate()}});
document.addEventListener("visibilitychange",async()=>{
 if(document.visibilityState==="visible"&&audioCtx&&audioCtx.state!=="running"){
  try{await audioCtx.resume()}catch(e){}
  audioUnlocked=audioCtx.state==="running";updateAudioGate();
 }
});
