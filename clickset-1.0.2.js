"use strict";
const VERSION="1.0.2", STORAGE_KEY="clickset_017_groups";
...
function scheduler(myRun){if(!playing||myRun!==runId)return;const song=flat[current].song,sec=60/Math.max(20,Math.min(400,Number(song.bpm)||120));while(nextBeatTime<audioCtx.currentTime+.12){const accented=Boolean(song.accentFirst)&&beatInBar===0;scheduleSound(effectiveSound(),nextBeatTime,accented?1.35:1);pulseAt(nextBeatTime,myRun);const beatsPerBar=Math.max(1,parseInt(String(song.meter||"4/4").split("/")[0],10)||4);beatInBar=(beatInBar+1)%beatsPerBar;nextBeatTime+=sec}schedulerTimer=setTimeout(()=>scheduler(myRun),20)}
