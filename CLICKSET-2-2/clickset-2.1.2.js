"use strict";
const VERSION="2.1.1", STORAGE_KEY="clickset_017_groups";
...
function routeToPan(route,node){
 if(route==="LR"){node.connect(audioCtx.destination);return}
 try{
  if(typeof audioCtx.createStereoPanner==="function"){const p=audioCtx.createStereoPanner();p.pan.value=route==="L"?-1:1;node.connect(p).connect(audioCtx.destination);return}
  const splitter=audioCtx.createChannelSplitter(2),merger=audioCtx.createChannelMerger(2);node.connect(splitter);
  if(route==="L")splitter.connect(merger,0,0);else splitter.connect(merger,0,1);merger.connect(audioCtx.destination);return
 }catch(e){console.warn("Routing estèreo no disponible; s’usa L + R",e);node.connect(audioCtx.destination)}
}
...
function updateRoutingUi(){
 const clickSel=$("clickRoute"),trackSel=$("trackRoute"),label=$("routingLabel"),compact=$("routingCompactLabel");
 ...
 if(compact)compact.textContent=`CLICK ${clickRoute} · PISTA ${trackRoute}`
}
...
function createAudioContext(){
 ...
 audioCtx.onstatechange=()=>updateAudioGate();
 return audioCtx
}
...
async function unlockAudioNow(showFeedback=false){
 ...
 audioUnlockPromise=(async()=>{
  try{
   ...
   const buffer=ctx.createBuffer(1,Math.max(1,Math.floor(ctx.sampleRate*.025)),ctx.sampleRate);
  ...
  }catch(e){console.warn("No s'ha pogut desbloquejar l'àudio",e);audioUnlocked=false;updateAudioGate();if(showFeedback)showAudioToast("No s’ha pogut activar el so");return false}
...
function loadSampleBuffers(){
 ...
 Object.entries(SAMPLE_FILES).forEach(([id,files])=>Object.entries(files).forEach(([kind,file])=>{
  ...
  jobs.push(fetch(`./${file}?v=20`,{cache:"force-cache"}).then(r=>{if(!r.ok)throw new Error(`${file}: ${r.status}`);return r.arrayBuffer()}).then(data=>audioCtx.decodeAudioData(data.slice(0))).then(buffer=>{sampleBuffers[key]=buffer}).catch(e=>console.warn("Sample",file,e)))
 }));
...
function loadClassicBuffer(){
 ...
 classicLoading=fetch("./click.wav?v=20",{cache:"force-cache"})
...
function reorderLabel(item){return item?.name||"Cançó sense nom"}
function reorderBpm(item){if(item?.children?.length)return `${item.children.length} parts`;return `${Number(item?.bpm)||120} BPM`}
function openReorderSetlist(){
 const sl=editingGroups?.[groupIndex]?.setlists?.[setlistIndex];if(!sl)return;
 reorderDraft=clone(sl.items||[]);renderReorderList();$("reorderSetlistModal").classList.add("open")
}
function closeReorderSetlist(){$("reorderSetlistModal").classList.remove("open");reorderDraft=null}
function renderReorderList(){
 const box=$("reorderList");if(!box||!reorderDraft)return;box.innerHTML="";$("reorderCount").textContent=`${reorderDraft.length} elements`;
 let dragIndex=null,pointerIndex=null;
 const move=(from,to)=>{if(from<0||to<0||from>=reorderDraft.length||to>=reorderDraft.length||from===to)return;const [it]=reorderDraft.splice(from,1);reorderDraft.splice(to,0,it);renderReorderList()};
 reorderDraft.forEach((item,i)=>{const row=document.createElement("div");row.className="reorderItem"+(item.children?" isMix":"");row.draggable=true;row.dataset.index=i;row.innerHTML=`<span class="reorderGrip">☰</span><span class="reorderNumber">${i+1}</span><span class="reorderTitle">${esc(reorderLabel(item))}</span><span class="reorderBpm">${esc(reorderBpm(item))}</span><div class="reorderMove"><button type="button" class="reorderUp" ${i===0?'disabled':''}>▲</button><button type="button" class="reorderDown" ${i===reorderDraft.length-1?'disabled':''}>▼</button></div>`;
 row.querySelector(".reorderUp").onclick=()=>move(i,i-1);row.querySelector(".reorderDown").onclick=()=>move(i,i+1);row.querySelector(".reorderNumber").title="Toca per indicar una posició";row.querySelector(".reorderNumber").onclick=()=>{const raw=prompt(`Moure “${reorderLabel(item)}” a quina posició?`,String(i+1));if(raw===null)return;const to=Math.max(1,Math.min(reorderDraft.length,parseInt(raw,10)||i+1))-1;move(i,to)};
 row.addEventListener("dragstart",e=>{dragIndex=i;row.classList.add("dragging");e.dataTransfer.effectAllowed="move"});row.addEventListener("dragend",()=>{dragIndex=null;row.classList.remove("dragging")});row.addEventListener("dragover",e=>{if(dragIndex===null)return;e.preventDefault();row.classList.add("dragOver")});row.addEventListener("dragleave",()=>row.classList.remove("dragOver"));row.addEventListener("drop",e=>{e.preventDefault();row.classList.remove("dragOver");if(dragIndex!==null)move(dragIndex,i)});
 const grip=row.querySelector(".reorderGrip");grip.addEventListener("pointerdown",e=>{if(e.pointerType==="mouse")return;pointerIndex=i;grip.setPointerCapture?.(e.pointerId);row.classList.add("dragging");e.preventDefault()});grip.addEventListener("pointermove",e=>{if(pointerIndex===null)return;const target=document.elementFromPoint(e.clientX,e.clientY)?.closest?.(".reorderItem");const to=Number(target?.dataset?.index);if(Number.isInteger(to)&&to!==pointerIndex){const [it]=reorderDraft.splice(pointerIndex,1);reorderDraft.splice(to,0,it);pointerIndex=to;renderReorderList()}});grip.addEventListener("pointerup",()=>{pointerIndex=null;row.classList.remove("dragging")});grip.addEventListener("pointercancel",()=>{pointerIndex=null;row.classList.remove("dragging")});
 box.appendChild(row)
 })
}
function saveReorderedSetlist(){const sl=editingGroups?.[groupIndex]?.setlists?.[setlistIndex];if(!sl||!reorderDraft)return;sl.items=reorderDraft;closeReorderSetlist();renderEditorItems(sl)}
function downloadJson(data,name){const blob=new Blob([JSON.stringify(data,null,2)],{type:"application/json"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=name;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},1000)}
function exportFullBackup(){const pack={format:"clickset-backup",formatVersion:1,appVersion:VERSION,exportedAt:new Date().toISOString(),groups:editingGroups||groups,settings:{clickRoute,trackRoute,appVolume,countdownEnabled}};downloadJson(pack,`clickset-backup-${new Date().toISOString().slice(0,10)}.clickset-backup`)}
async function importFullBackup(file){try{const pack=JSON.parse(await file.text());if(pack.format!=="clickset-backup"||!Array.isArray(pack.groups))throw new Error("Format incorrecte");if(!confirm("Restaurar aquest backup substituirà tots els grups i repertoris actuals. Continuar?"))return;groups=normalize(pack.groups);editingGroups=clone(groups);if(pack.settings){setRoutes(pack.settings.clickRoute||clickRoute,pack.settings.trackRoute||trackRoute);if(Number.isFinite(Number(pack.settings.appVolume))){appVolume=Math.max(0,Math.min(1,Number(pack.settings.appVolume)));localStorage.setItem("clickset_app_volume",String(appVolume))}if(typeof pack.settings.countdownEnabled==="boolean"){countdownEnabled=pack.settings.countdownEnabled;localStorage.setItem(COUNTDOWN_KEY,String(countdownEnabled))}}save();alert("Backup restaurat correctament.");$("editor").classList.remove("open");showGroups()}catch(e){console.error(e);alert("No s’ha pogut restaurar: el fitxer no és un backup complet de ClickSet.")}}
...
if($("modifySetlistBtn"))$("modifySetlistBtn").onclick=openReorderSetlist;
if($("cancelReorder"))$("cancelReorder").onclick=closeReorderSetlist;
if($("saveReorder"))$("saveReorder").onclick=saveReorderedSetlist;
if($("openAudioRoutingBtn"))$("openAudioRoutingBtn").onclick=()=>{$("audioRoutingModal").classList.add("open");updateRoutingUi()};
if($("closeAudioRouting"))$("closeAudioRouting").onclick=()=>$("audioRoutingModal").classList.remove("open");
if($("exportBackupBtn"))$("exportBackupBtn").onclick=exportFullBackup;
if($("importBackupBtn"))$("importBackupBtn").onclick=()=>{$("backupImportInput").value="";$("backupImportInput").click()};
if($("backupImportInput"))$("backupImportInput").onchange=e=>{const f=e.target.files?.[0];if(f)importFullBackup(f)};
...
// Service worker disabled in 2.1.2 to force a clean deployment and avoid stale cached files.
