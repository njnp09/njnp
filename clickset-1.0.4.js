"use strict";
const VERSION="1.0.4", STORAGE_KEY="clickset_017_groups";
...
async function change(d){
 ...
 const wasPlaying=playing;
 if(wasPlaying)stop();
 current=(current+d+flat.length)%flat.length;render();
 if(wasPlaying)await start(true)
}
...
function scheduler(myRun){
 ...
 while(nextBeatTime<audioCtx.currentTime+.12){
  const accented=Boolean(song.accentFirst)&&beatInBar===0;
  scheduleSound(effectiveSound(),nextBeatTime,accented?1.35:1);
 ...
 schedulerTimer=setTimeout(()=>scheduler(myRun),20)
}
async function start(immediateFirstBeat=true){
 ...
 const song=flat[current].song;
 trackBuffer=null;
 if(song.trackId){try{await loadCurrentTrackBuffer()}catch(e){console.warn("No s’ha pogut carregar la pista",e)}}
 playing=true;runId++;beatInBar=0;
 ...
 const firstBeatTime=audioCtx.currentTime+(immediateFirstBeat?.003:.018);
 const accented=Boolean(song.accentFirst);
 scheduleSound(effectiveSound(),firstBeatTime,accented?1.35:1);
 pulseAt(firstBeatTime,runId);
 ...
 if(trackBuffer){
  trackSource=track(audioCtx.createBufferSource());trackGainNode=audioCtx.createGain();trackSource.buffer=trackBuffer;
  ...
  const trackStart=firstBeatTime+(countInBars*beatsPerBar*sec);
